...
-- Expecting listIndex (number) to match simple array structure of PriorityLists
function UI_PriorityOverride:ShowPriorityOverrideWindow(listIndex)
    if self.priorityOverrideFrame then
        self.priorityOverrideFrame:Hide()
    end

    local db = DesolateLootcouncil.db.profile
    -- Safety check for current DB structure (Array)
    local list = db.PriorityLists[listIndex]
    if not list then
        DesolateLootcouncil:DLC_Log("Error: Priority List #" .. tostring(listIndex) .. " not found.")
        return
    end

    local listName = list.name or ("List " .. listIndex)
    ...
    local function RefreshList()
        ...
        local currentList = db.PriorityLists[listIndex].players -- Access players array inside list object
        -- Safety check
        ...
        for i, name in ipairs(currentList) do
            ...
            row:SetScript("OnDragStop", function(self)
                ...
                if newIndex ~= i then
                    ...
                    DesolateLootcouncil:DLC_Log(string.format("Moved %s to rank #%d in %s list.", player, newIndex,
                        listName))
                end
